...
```{r} flies <- c(183.2, 149.0, 154.0, 167.2, 187.2, 158.0, 143.0 )}
```
...
```{r} strawberries <- c(239, 176, 235, 217, 234, 216, 318, 190, 181, 225) z.test(x=strawberries,mu = 205, sigma.x=sqrt(1500))}
```
...
```{r} t.test(x=strawberries,mu = 205)}
```
...
```{r} wilcox.test(strawberries, mu=205)}
```
...
```{r} SIGN.test(strawberries, md=205)}
```
...
```{r} snakes <- c(0.9, 1.4, 1.2, 1.2, 1.3, 2.0, 1.4, 1.6) t.test(snakes, mu=1.4)}
```
...
```{r} sport <- read.table("http://www.statsci.org/data/oz/ais.txt", header = T,                      stringsAsFactors = T)}
```

```{r} library(MKinfer) boot.t.test(sport[sport$Sex == "male", "Wt"], mu= 85.9)}
```

...
The : means makes a list from the bottom number to the top number,
including all the integers inbetween
...
Length of x returns the number of elements in a vector. max gives the
...
The first creates a sequence from -1 to 2 by ones. The second does the
same. The third creates a sequence from 1 to 2 then applies a negative
to it.
...
Dim returns the number of rows and columns in a dataframe! So sleep has
r dim(sleep)\[1\] rows.
...
This is showing a comparison between primates and the other species
inside the variable data set in "sleep."
...
The data has a vertical path that was able to be generated well. There
are also the outliers that were a part of it, though removed. The data
